...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const chargerDonnees = async () => {
      setLoading(true);
      try {
        const [dataBeneficiaires, dataInventaire, dataPacks] = await Promise.all([
          getBeneficiaires(),
          getInventaire(),
          getPacks()
        ]);
      ...
      <div className="bg-white shadow-md border-b-2 border-emerald-200">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-emerald-800">Admin - Zakat al-Fitr</h1>
              <p className="text-sm text-gray-600">Système de gestion complet</p>
            </div>
